...
bool FORMULA_ArgumentConstantValue::FillFromText(const char* str, int &offset)
{
	...
	//offset += BS_FloatWOZeros(m_const_value, 6).GetLength(); // ОПАСНОЕ МЕСТО :)
	return true;
}
...
{
	return TXT("Группа формул (%d шт)") << m_formulas.size();
}
...
FORMULA_Formula* FORMULA_Formula::CreateFromText(const char* str, int &offset)
{
	...
	for (int i = 1; i <= formula->GetArgumentsNum(); i++)
	{
        ...
        if (prev_offset == offset) // не считали "A<NULL>"
		{
			delete formula;
			return NULL;
		}
...
{
	FormulaDelta():
	FORMULA_Formula("Приращение(%s1)"), m_start(true)
	{
		m_num_in_map = 7;
	}
...
protected:
	...
	virtual float Function(byte &status)
	{
		...
		if (status == FORMULA_CALC_STATUS_COMPLETE)
		{
			...
			if (status == FORMULA_CALC_STATUS_COMPLETE)
			{
				if (m_window_size != w) // изменение размера окна суммирования
				{
					if (w <= 0)
						return new_val;
					...
					m_window_size = w;
					m_collected = min(m_collected, m_window_size - 1); // на одну меньше окна, чтобы было куда дописать
				}
				if (m_collected >= m_window_size) // буфер заполнен
				{
					memcpy(m_window_data, m_window_data + 1, (m_window_size - 1)*sizeof(float));
					m_collected = m_window_size - 1;
				}
				*(m_window_data + m_collected) = new_val; // записываем новые данные
				m_collected++;
			}
			else
			{
...
{
	FormulaSuckenDelta():
		FORMULA_Formula("ПНК(%s1)"), m_start(true)
	{
		m_num_in_map = 9;
		m_last_value = 0;
	}
